...
* text: 此处的文本内容将分解为逐个单词，这些单词作为词云的元素。单词出现频率越高，在词云上描绘得越大。如果此处没有任何文字输入，将使用演示文本生成图片。
...
* margin: 空白边大小。
* font_path: 字体文件路径。建议使用ttf格式。如此处指定的字体文件无效，则启用自带的Alibaba-PuHuiTi-Heavy.ttf（该字体版权归属阿里巴巴（中国）有限公司，允许任何个人和企业免费使用。）
...
* stopwords: 屏蔽词。设定在此处的单词不会在图中出现，除非在重点词里有重新设定。每个词之间用逗号(中英文均可)或空格分开。
